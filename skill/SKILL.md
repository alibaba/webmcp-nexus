...
WebMCP 构建插件（Vite/Webpack plugin）通过 ts-morph 从 `registerGlobalTools(...)` / `useWebMcpTools(...)` 的调用位置**向上追踪**工具函数的定义，提取 JSDoc 与参数类型，在构建阶段把 `__webmcpSchema`（description / inputSchema / annotations）注入到每个函数对象上。SDK 在运行时读取该字段向 `navigator.modelContext` 注册。
...
| #   | 约束                                                                                                                                                                                                                                                                  |
| --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| M1  | 工具函数必须**可被追踪**。两种可追踪形式：① 作为对象字面量属性传入 `registerGlobalTools({ fn })` / `useWebMcpTools({ fn })`；② 在 `import * as api from './module'` 的 `./module` 里作为**具名导出**（避免 `export default`，否则工具名会被解析成字符串 `"default"`） |
...
组件 / 路由级工具则在对应组件内调用 `useWebMcpTools`（见 §5.2）。
...
| `useWebMcpTools`      | 组件 / 路由级    | React 组件函数内作为 Hook 调用，自动注销 |
